...
def build_query_set(
    gt_tracks_px: np.ndarray,   # (N, F, 2), pixel (x, y)
    occluded: np.ndarray,       # (N, F), bool
    query_mode: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Build TAP-Vid query arrays from per-track annotations.

    Returns:
        query_points: (Nq, 3), [t, y, x]
        target_points: (Nq, F, 2), [x, y]
        target_occluded: (Nq, F)
    """
    query_points = []
    tracks = []
    occs = []
    ...
    if query_mode == 'first':
        for n in range(gt_tracks_px.shape[0]):
            visible_frames = np.where(~occluded[n])[0]
            if len(visible_frames) == 0:
                continue
            t_q = int(visible_frames[0])
            x_q = float(gt_tracks_px[n, t_q, 0])
            y_q = float(gt_tracks_px[n, t_q, 1])
            query_points.append([t_q, y_q, x_q])
            tracks.append(gt_tracks_px[n])
            occs.append(occluded[n])

    elif query_mode == 'strided':
        for t_q in range(0, gt_tracks_px.shape[1], 5):
            visible_indices = np.where(~occluded[:, t_q])[0]
            for n in visible_indices:
                x_q = float(gt_tracks_px[n, t_q, 0])
                y_q = float(gt_tracks_px[n, t_q, 1])
                query_points.append([t_q, y_q, x_q])
                tracks.append(gt_tracks_px[n])
    ...
    if len(query_points) == 0:
        raise ValueError(f'No valid queries for query_mode={query_mode}')

    return (
        np.array(query_points, dtype=np.float32),
        np.stack(tracks, axis=0).astype(np.float32),
        np.stack(occs, axis=0).astype(np.bool_),
    )
...
def evaluate_video(
    ...
    # build query_points in [t, y, x] format using the requested mode
    metric_query_mode = query_mode
    query_points_arr, gt_tracks_query, occluded_query = build_query_set(
        gt_tracks_px=gt_tracks_px,
        occluded=occluded,
        query_mode=metric_query_mode,
    )
    ...
    for n in tqdm(range(N_queries), desc=f'  Tracking {vid_name}', leave=False):
        ...
        if n < 0:   # save at most 5 videos per video for quick inspection, can be adjusted
            ...
            visualize_tracking(
                ...
                gt_track=gt_tracks_query[n],       # (F, 2) pixel (x, y)
                video_tensor=video,
                query_frame=t_q,
                query_y=y_q,
                query_x=x_q,
        ...
        # print(gt_tracks_query[n])
        # print(occluded_query[n])
        # print(pred_xy)
        # print(pred_occ)
        # break

    pred_tracks_arr = np.stack(all_pred_tracks, axis=0)       # (Nq, F, 2)
    pred_occ_arr = np.stack(all_pred_occlusions, axis=0)      # (Nq, F)

    # compute metrics — add batch dim=1
    query_points_b = query_points_arr[np.newaxis]          # (1, Nq, 3)
    gt_tracks_b    = gt_tracks_query[np.newaxis]           # (1, Nq, F, 2)
    gt_occluded_b  = occluded_query[np.newaxis]            # (1, Nq, F)
    pred_tracks_b  = pred_tracks_arr[np.newaxis]           # (1, Nq, F, 2)
    pred_occ_b     = pred_occ_arr[np.newaxis]              # (1, Nq, F)
...
def main(args):
    ...
    metric_query_mode = args.query_mode
    ...
    results = {
        ...
        'config': {
            ...
            'eval_size':  args.eval_size,
            'query_mode': metric_query_mode,
            'query_stride': 5 if metric_query_mode == 'strided' else None,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--query_mode', type=str, default='first',
