...
python evaluate1.py \
    ...
    --output_dir /content/result/staticquery
"""
...
def track_point(
    ...
    def get_feature(f_idx, y, x):
        ...
        feat_up = F.interpolate(
            feat_low, size=(img_H, img_W), mode='bilinear', align_corners=False
        )
    ...
    def process_frame(q_vec, target_f_idx, ref_y, ref_x, local_window=None, pixel_window=3):
        ...
        target_feat_up = F.interpolate(
            target_feat_low, size=(img_H, img_W), mode='bilinear', align_corners=False
        )
...
def visualize_tracking(
    ...
    """Draw predicted and ground-truth tracks and save an mp4."""
    ...
    for t in range(F_num):
        ...
        cv2.circle(frame, (int(round(pred_x)), int(round(pred_y))), dot_radius, (0, 255, 0), -1)

        gx = int(round(float(gt_track[t, 0])))
        gy = int(round(float(gt_track[t, 1])))
        cv2.circle(frame, (gx, gy), dot_radius, (0, 0, 255), -1)

        if t == query_frame:
            qx = int(round(query_x))
            qy = int(round(query_y))
            cv2.circle(frame, (qx, qy), dot_radius, (255, 0, 0), -1)
            cv2.circle(frame, (int(round(pred_x)), int(round(pred_y))), dot_radius + 5, (255, 255, 255), 1)
...
def evaluate_video(pt_path: str, output_dir: Path):
    ...
    img_H, img_W = int(meta['eval_size']), int(meta['eval_size'])#int(video.shape[1]), int(video.shape[2])
